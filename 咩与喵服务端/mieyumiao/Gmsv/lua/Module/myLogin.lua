...
function MyLogin_LoginEvent(player)
	...
	if (WelcomeMessage ~= nil) then --欢迎词
		NLG.SystemMessage(player,WelcomeMessage);
	end
...
function inityddlNpc()
	if (yddlnpc == nil) then
		yddlnpc = NL.CreateNpc(nil, "inityddlNpc_Init");
...
function yddlnpcMsg(_me,_tome)
	if (NLG.CanTalk(_me,_tome) == true) then
		str_ChangeWindow = "领取"..tbl_In_situsetting.defcount.."次原地登录成功。";
	    tbl_dropinfo[Playerkey(_tome)].leftCount = tbl_In_situsetting.defcount; 
		NLG.ShowWindowTalked(_tome,_me,%窗口_信息框%,%按钮_关闭%,1,NLG.c(str_ChangeWindow));
		return;
	end
end
