...
function OnlinePlayer_LoginGateEvent(_PlayerIndex)
	...
	if(Char.GetData(_PlayerIndex,%对象_PK开关%)==0)then
		--Char.SetData(Index,%对象_PK开关%,1);
		--NLG.UpChar(Index);
		--Battle.SetType(Index,%战斗_普通%);
		--Battle.SetGainMode(Index,%战奖_普通%);
	end
...
function All_Online(Index)
	local tPlayer = {
							CdKey = Char.GetData(Index, %对象_CDK%),
