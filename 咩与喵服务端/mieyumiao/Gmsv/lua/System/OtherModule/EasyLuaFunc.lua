...
function isLv1Pet(checkPet)
	...
	if(Char.GetData(checkPet,%对象_等级%)==1 and Char.GetData(checkPet,%对象_种族%)~=9)then
		if((Char.GetData(checkPet,%对象_原名%)=="迷你蝙蝠") or (Char.GetData(checkPet,%对象_原名%)=="哥布林") or (Char.GetData(checkPet,%对象_原名%)=="盗贼") or (Char.GetData(checkPet,%对象_原名%)=="小石像怪"))then
			return false;
		end
		return true;
	end
...
function getTabKey(_PlayerIndex)
	local tCdKey = Char.GetData(_PlayerIndex, %对象_CDK%)
